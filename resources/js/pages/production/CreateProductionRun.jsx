...
import { getSelectedBranchId } from '../../services/api';

const fmt = (n) => window.formatCurrency(n, { minimumFractionDigits: 2 });

export default function CreateProductionRun() {
    ...
    // Load formulas and warehouses on mount
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                // Formulas response is paginated { data: [...] }, warehouses is direct array
                ...
                // Filter only active formulas on the client side
                const activeFormulas = Array.isArray(formulasList) 
                    ? formulasList.filter(f => f.is_active !== false)
                    : [];
                
                console.log('Loaded formulas:', activeFormulas);
                ...
                toast.error('Failed to load formulas and warehouses');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleFormulaChange = (e) => {
        ...
        const formula = formulas.find((f) => f.id === Number(id));
        if (formula) {
            setOutputProduct(formula.name + ' Feed');
        } else {
            setOutputProduct('');
        }
    };
    ...
    const handleSubmit = async () => {
        if (!selectedFormulaId || !targetOutput || !selectedWarehouseId) {
        ...
        try {
            ...
            const response = await productionAPI.create({
                ...
                warehouse_id: Number(selectedWarehouseId),
                branch_id: Number(getSelectedBranchId()),
                production_date: productionDate,
                target_quantity: Number(targetOutput),
                notes: outputProduct ? `Output: ${outputProduct}` : null,
            });
            toast.success(`Production run ${response.data?.production_number || ''} created. Status: Planned`);
            navigate('/production');
        } catch (error) {
            console.error('Error creating production run:', error);
            const message = error.response?.data?.message || 'Failed to create production run';
            toast.error(message);
        } finally {
            setSubmitting(false);
        }
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold text-slate-900">New Production Run</h1>
                </div>
            </div>
            ...
            <Card>
                <CardBody>
                    <h2 className="text-lg font-semibold text-slate-800 mb-4">Planning</h2>
                    {loading ? (
                        <div className="text-center py-8 text-slate-500">Loading formulas and warehouses...</div>
                    ) : (
                            ...
                            <div className="mb-4 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <span>⚠</span>
                                        <span>Insufficient stock for {insufficientCount} item{insufficientCount > 1 ? 's' : ''}. Production may be limited.</span>
                                    </div>
                                    <Button 
                                        variant="outline" 
                                        size="sm"
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {inputLines.map((line, idx) => (
                                        <tr key={idx} className="border-b border-slate-100">
                                            ...
                                            <td className="py-3 pr-4 text-right text-slate-700">{Number(line.plannedQty).toLocaleString('en-NG')}</td>
                                ...
                                <tfoot>
                                    <tr className="border-t-2 border-slate-300">
                                        ...
                                        <td className="pt-3 pr-4 text-right font-semibold text-slate-900">{Number(totalPlannedInput).toLocaleString('en-NG')}</td>
                    ...
                    <CardBody>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="mt-2 rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">
                                Industry avg: {window.getCurrencySymbol()}450–550/kg
                            </div>
                        </div>
                    </CardBody>
                </Card>
            ...
            <div className="flex items-center justify-end gap-3">
                ...
                <Button
                    ...
                    disabled={!selectedFormula || !targetOutput || !selectedWarehouseId || submitting}
                >
